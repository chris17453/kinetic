...
test.describe.serial('Kinetic local smoke flows', () => {
  ...
  test('catalog, builder, and report viewer load with seeded reports', async ({ page }) => {
    ...
    await expect(page.getByPlaceholder('Untitled Report')).toBeVisible();
    await expect(page.getByRole('button', { name: /create report/i })).toBeVisible();

    await page.goto(`/reports/${seed.report.id}/edit`);
    await page.getByRole('button', { name: /visualization/i }).click();
    await expect(page.getByText('Field Wells').first()).toBeVisible();
    await expect(page.getByText('Canvas Layout').first()).toBeVisible();
  ...
  test('workspace, dataset, dashboard, profile, integrations, and refresh pages load', async ({ page }) => {
    ...
    await expect(page.getByText(seed.dashboard.name!).first()).toBeVisible();
    await expect(page.getByRole('button', { name: /view/i })).toHaveClass(/btn-primary/);
    await expect(page.getByText('Revenue YTD')).toBeVisible();
    await expect(page.getByText(/region Equals North/)).toBeVisible();
    ...
    await expect(page.getByRole('heading', { name: 'Enterprise Integrations' })).toBeVisible();
